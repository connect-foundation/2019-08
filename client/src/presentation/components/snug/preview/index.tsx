...
export const Preview: React.FC<AppChannelMatchProps & PropsType> = props => {
  ...
  useEffect(() => {
    (async function () {
      try {
        const channel = await application.services.channelService.getChannelById(pathParameter.channelId!);
        if (!!channel) {
  ...
  async function join() {
    try {
      const {channel} = await application.services.channelService.join(pathParameter.channelId!);
      setIsParticipated(!!channel);
      channelDispatch && channelDispatch({type: "MULTI", channels: [channel]});
    } catch (error) {
