...
export class Chatter {
  ...
  private constructor(postNotifier?: Notifier<Post>, replyNotifier?: Notifier<Post>) {
  ...
  private async findProfileAndRoom(profileId: string, roomId: string): Promise<object> {
    return {
  ...
  public async post(contents: string, profileId: string, roomId: string): Promise<PostInfo> {
  ...
  public async reply(parentPostId: string, profileId: string, contents: string, roomId: string): Promise<PostInfo> {
    const profileAndRoom = await this.findProfileAndRoom(profileId, roomId);
    const parent = await Post.findOneOrFail(parentPostId);
    const post = await Post.save({contents, parent, ...profileAndRoom} as Post);
  ...
  @Transactional({propagation: Propagation.REQUIRED, isolationLevel: IsolationLevel.READ_COMMITTED})
  public async findPosts(channelId: string, postId: string, size: string, order: string): Promise<PostInfo[]> {
  ...
  public async findReplies(parentPostId: string): Promise<ReplyInfo> {
    ...
    const replies = await Post.findReplies(parentPost, new Order().add("id", "ASC"));
