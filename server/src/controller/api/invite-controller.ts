...
export const verify = async (request: Request, response: Response, next: NextFunction): Promise<void> => {
...
export const responseBySnug = (request: Request, response: Response): Response => {
  const {agree} = request.body;
  ...
  if(!!snug) {
    ...
    return response.status(CONFLICT)
            .json(ResponseForm.of<object>(ALREADY_JOINED_SNUG, {snug: inviteSnug}));
  }
