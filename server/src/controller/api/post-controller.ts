...
export const create = async (request: Request, response: Response): Promise<Response> => {
  ...
  try {
    ...
    const postInfo = await Chatter
            .fromPost(postNotifier)
            .post(contents, profileId, roomId);
    return response.status(CREATED).json(ResponseForm.of<PostInfo>(CREATE_POST, postInfo));
  } catch (error) {
    return response.status(NOT_FOUND).json(ResponseForm.of(INVALID_POST_REQUEST));
  }
};

export const findPosts = async (request: Request, response: Response): Promise<Response> => {
  const {channelId} = request.params;
  const {postId, size, order} = request.query;
  try {
    const postInfos = await Chatter
            .create()
            .findPosts(channelId, postId, size, order);
    return response.status(OK).json(ResponseForm.of<object>(SUCCESS_FOUND_POSTS, {posts: postInfos}));
  } catch (error) {
    return response.status(NOT_FOUND).json(ResponseForm.of<object>(NOT_FOUND_POSTS, {posts: []}));
  }
};
...
export const reply = async (request: Request, response: Response): Promise<Response> => {
  ...
  try {
    ...
    const postInfo = await Chatter
            .fromReply(replyNotifier)
            .reply(postId, profileId, contents, roomId);
    return response.status(CREATED).json(ResponseForm.of<PostInfo>(CREATE_REPLY, postInfo));
  } catch (error) {
    return response.status(NOT_FOUND).json(ResponseForm.of(INVALID_REPLY_REQUEST));
  }
};
...
export const findReplies = async (request: Request, response: Response): Promise<Response> => {
  const {postId} = request.params;
  try {
    const replyInfo = await Chatter
            .create()
            .findReplies(postId);
    return response.status(OK).json(ResponseForm.of<ReplyInfo>(SUCCESS_FOUND_REPLIES, replyInfo));
  } catch (error) {
